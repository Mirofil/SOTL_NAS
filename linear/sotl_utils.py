...
def sotl_gradient(
    model, criterion, xs, ys, weight_buffer: Sequence, w_lr:float, T:int, order=1, hvp="exact", normalize_a_lr=True
) -> Sequence:
    total_arch_gradient = None
    weight_buffer = weight_buffer[-(min([len(weight_buffer)-len(xs),len(weight_buffer)- len(ys)])):]
    

    if order is None or order <= 0:
        order = min([len(weight_buffer), len(xs), len(ys)])
        if len(weight_buffer) != len(xs):
            print("SCERAM")
    if (
        len(weight_buffer) == 1
        ...
        for i in range(
            len(weight_buffer) - 1, max(0, len(weight_buffer) - 1 - order), -1
        ):
            ...
            loss2 = criterion(
                model(xs[i], weight_buffer[i - 1][0], model.fc1.alphas), ys[i]
            )
            hessian_matrix = hessian(
                loss2 * 1, weight_buffer[i - 1][0], model.fc1.alphas
            ).reshape(
                model.fc1.weight.size()[1], model.fc1.alphas.size()[1]
            )  # TODO this whole line is WEIRD
            ...
            for i, a_grad in enumerate(da):
                total_arch_gradient[i] += a_grad + (
        ...
        for i in range(
            ...
            # w+ = w + eps*dw`
            with torch.no_grad():
                for p, d in zip(weight_buffer[i - 1], dw):
                    p.add_(eps * d)
