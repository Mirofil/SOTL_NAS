...
def sotl_gradient(model, criterion, xs, ys, weight_buffer, w_lr, order=1, hvp = "exact"):
    ...
    if hvp == "exact":
        ...
        for i in range(len(weight_buffer)-1, max(0, len(weight_buffer)-1-order), -1):
            ...
            total_arch_gradient_local = [-w_lr*(p - n) / (2.0 * eps) for p, n in zip(dalpha_pos, dalpha_neg)]
