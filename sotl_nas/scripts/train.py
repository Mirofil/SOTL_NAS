...
# python sotl_nas/scripts/train.py --model_type=MNIST --dataset=MNIST --dry_run=False --T=1 --w_warm_start=0 --grad_outer_loop_order=-1 --grad_inner_loop_order=-1 --mode=bilevel --device=cuda --extra_weight_decay=0.0001 --w_weight_decay=0 --arch_train_data=val
# python sotl_nas/scripts/train.py --model_type=max_deg --epochs 1 --steps_per_epoch=None --dataset=fourier --dry_run=True --grad_outer_loop_order=-1 --grad_inner_loop_order=-1 --mode=bilevel --device=cpu --ihvp=exact --inv_hess=exact --hvp=exact --rand_seed 1 --arch_train_data sotl --optimizer_mode=manual --T=3 --recurrent False --w_lr=1e-1 --a_lr=1e-3 --adaptive_a_lr=False --w_optim=HyperSGD
# python sotl_nas/scripts/train.py --model_type=rff_bag --epochs 50 --dataset=MNISTrff --dry_run=True --grad_outer_loop_order=-1 --grad_inner_loop_order=-1 --mode=bilevel --device=cuda --ihvp=exact --inv_hess=exact --hvp=exact --rand_seed 1 --arch_train_data sotl --optimizer_mode=autograd --loss=ce --T=2 --recurrent True --a_weight_decay 0.0 --a_lr=0.1 --w_weight_decay 0.0001 --train_arch=True --w_lr=1.0 --optimizer_mode=autograd
...
# python sotl_nas/scripts/train.py --model_type=log_reg --dataset=MNIST --dry_run=False --T=2 --w_warm_start=0 --grad_outer_loop_order=-1 --grad_inner_loop_order=-1 --mode=bilevel --device=cpu --w_weight_decay=0 --arch_train_data=sotl --alpha_lr=0.001 --w_lr=1e-3 --a_lr=1e-2 --alpha_lr=1e-3 --optimizer_mode=autograd --loss=ce --a_weight_decay=0
# python sotl_nas/scripts/train.py --cfg=sotl_nas/configs/max_deg/lin_reg.py
# python sotl_nas/scripts/train.py --cfg=sotl_nas/configs/lr/mnist_logreg.py --alpha_lr=0.01 --alpha_lr=None --w_decay_order=0 --model_type=LinearSupernetRFF --dataset=MNISTrff
...
# python sotl_nas/scripts/train.py --cfg=sotl_nas/configs/lr/mnist_mlp.py --alpha_lr_reject_strategy=half --T=25 --train_arch=True --w_lr=0.01 --w_momentum=0.9 --w_optim=HyperSGD --alpha_lr=0.01--mode=bilevel --model_type=MLP2 --a_lr=0.01 --a_weight_decay=0.1

# python sotl_nas/scripts/train.py --cfg=sotl_nas/configs/lr/mnist_vgg.py --T=3 --w_momentum=0.9 --w_optim=HyperSGD --a_lr=0.05 --alpha_lr=0.1 --a_decay_scheduler=linear --a_weight_decay=0.1
...
import wandb
from sotl_nas.configs.defaults import cfg_defaults
...
from sotl_nas.training.train_loop import train_bptt, valid_func
...
from sotl_nas.utils.metrics import SumOfWhatever, ValidAccEvaluator, obtain_accuracy
from sotl_nas.utils.train import (calculate_weight_decay, compute_auc,
                         compute_train_loss, get_criterion, get_optimizers,
                         hinge_loss, inverse_softplus, reconstruction_error,
                         switch_weights)
...
def main(cfg=None, **kwargs):
    ...
    if config["train_arch"] is not False:
        assert 'hyper' in config["w_optim"].lower()
