...
from sotl_nas.training.train_steps import arch_step, train_step
from sotl_nas.utils.features import choose_features
from sotl_nas.utils.general_utils import (data_generator, eval_features,
                                          featurize, hessian, jacobian,
                                          prepare_seed)
from sotl_nas.utils.log_utils import AverageMeter, wandb_auth
from sotl_nas.utils.metrics import (SumOfWhatever, ValidAccEvaluator,
                                    obtain_accuracy)
from sotl_nas.utils.train import (calculate_weight_decay, compute_auc,
                                  compute_train_loss, get_criterion,
                                  get_optimizers, hinge_loss,
                                  reconstruction_error, switch_weights)
    ...
    for epoch in tqdm(range(epochs), desc='Iterating over epochs', total = epochs):
        ...
        for batch_idx, (batch, val_batch) in tqdm(enumerate(zip(train_loader, itertools.cycle(val_loader))), desc = "Iterating over batches", total = len(train_loader), disable = not config["progress_bar"]):
            ...
            if mode == "bilevel":
                ...
                for param_group_idx, param_group in enumerate(prerollout_w_optim_state_dict["param_groups"]):
                    for k, v in param_group.items():
                        if type(v) is list: # The momentum buffer for HyperSGD is a list at this point
                            try:
                                for nested_dict in v:
                                    for k2, v2 in nested_dict.items():
                                        if torch.is_tensor(v2):
                                            nested_dict[k2] = nested_dict[k2].detach()
                            except:
            ...
            if (mode == "bilevel" and epoch >= w_warm_start and batch_idx % arch_update_frequency == 0) or (mode == "joint" and train_arch == False):
                ...
                for i in range(min(bilevel_w_steps, len(xs))):
                    ...
                    train_loss.update(unreg_loss.item())
...
def train_supernet_rff(
    ...
    **kwargs
    ...
    suffixed_name = model.model_type + f"l={config['l']}" + log_suffix
    ...
    for epoch in tqdm(range(epochs), desc='Iterating over epochs', total = epochs):
            ...
            for i in range(min(bilevel_w_steps, len(xs))):
                ...
                with torch.no_grad():
                    for g, (w_name, w) in zip(grads, model.named_weight_params()):
                        w.grad = g
                if grad_clip is not None:
                    clip_coef = torch.nn.utils.clip_grad_norm_(model.weight_params(), grad_clip)
                    print(f"Clip coef {clip_coef}")

                w_optimizer.step()
                w_optimizer.zero_grad()

                metrics["train_loss"][epoch].append(-unreg_loss.item())
                if dataset_cfg["dset_val"] is not None:
                    val_acc_top1, val_acc_top5, val_loss = val_acc_evaluator.evaluate(model, criterion)
                    metrics["val_loss"][epoch].append(-val_loss)
                    if val_acc_top1 is not None:
        ...
        tqdm.write(
            "Epoch: {}, Batch: {}, Train loss: {}, Train acc: {}".format(
                epoch,
                true_batch_idx,
                train_loss.avg,
                train_acc.avg
        ...
        for _ in range(len(train_loader)):
            metrics["val_acc"][epoch].append(val_results_acc)
            metrics["val_loss"][epoch].append(val_results_loss)
            metrics["test_acc"][epoch].append(test_acc_results.avg)
            metrics["test_loss"][epoch].append(test_results.avg)

        tqdm.write("Epoch: {}, Val Loss: {}, Val Acc: {}, Test Loss: {}, Test Acc: {}".format(epoch, val_results.avg, val_acc_results.avg, test_results.avg, test_acc_results.avg))
        to_log = {**to_log, "val_loss": val_results_loss, "val_acc": val_results_acc, "test_loss": test_results.avg, "test_acc": test_acc_results.avg, "epoch": epoch}
        to_log = {k:v for k,v in to_log.items() if v is not None}
        wandb.log({suffixed_name:{dataset:{**to_log}}})
